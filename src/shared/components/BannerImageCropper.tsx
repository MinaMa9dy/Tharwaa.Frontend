...
export default function BannerImageCropper({ onCrop }: BannerImageCropperProps) {
  ...
  const getDimensions = () => {
    ...
    const containerWidth = containerRef.current.clientWidth;
    const containerHeight = containerWidth / 3; // Aspect ratio 3:1
  ...
  const triggerCrop = () => {
    ...
    // High quality banner export resolution
    canvas.width = 1200;
    canvas.height = 400;
    ...
    <div className="space-y-4">
      {!imageSrc ? (
        <div className="border-2 border-dashed border-slate-300 hover:border-primary rounded-2xl p-6 text-center cursor-pointer transition-all bg-slate-50 hover:bg-slate-100/50">
          ...
          <label htmlFor="banner-file-input" className="cursor-pointer space-y-2 block">
            ...
            <span className="text-xs font-black text-slate-700 block">
              {locale === 'ar' ? 'اختر صورة البانر لبدء قصها' : 'Select banner image to start cropping'}
            </span>
            <span className="text-[10px] font-bold text-slate-400 block">
              {locale === 'ar' ? 'يدعم صيغ JPG، PNG، إلخ. ويتم تحويلها لـ WebP' : 'Supports JPG, PNG, etc. Auto-converts to WebP'}
            </span>
          </label>
        </div>
      ) : (
        <div className="space-y-4">
          ...
          <div className="relative w-full overflow-hidden rounded-2xl border border-slate-300 shadow-inner bg-slate-200 select-none">
            <div
              ref={containerRef}
          ...
          <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200/50 space-y-2">
            ...
            <p className="text-[10px] font-bold text-slate-400 text-center flex items-center justify-center gap-1">
              ...
              {locale === 'ar' ? 'اسحب الصورة لتعديل تموضعها داخل الإطار' : 'اسحب الصورة لتعديل تموضعها داخل الإطار'}
            </p>
          </div>

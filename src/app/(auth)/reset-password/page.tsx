...
function ResetPasswordForm() {
  ...
  if (!userId || !token) {
    return (
      <div className="space-y-6 text-center">
        <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 text-sm text-amber-700 font-semibold flex items-center justify-center gap-2">
          <WarningIcon className="w-4 h-4 text-amber-600 flex-shrink-0" />
          <span>{locale === 'ar' ? 'رابط إعادة التعيين غير صالح أو مفقود.' : 'Invalid or missing password reset link.'}</span>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {successMsg && (
        <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-sm text-emerald-700 font-semibold animate-fade-in flex items-center gap-2">
